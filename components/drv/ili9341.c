...
static void ili9341_rotate_screen(int32_t width, int32_t height, const lv_color_t * color_map, lv_color_t * rot_map);
...
void ili9431_flush(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const lv_color_t * color_map)
{
  ...
  y2 = temp;
	...
	uint32_t size = (x2 - x1 + 1) * (y2 - y1 + 1);
  lv_color_t * new_map = malloc(size * 2);

  ili9341_rotate_screen(y2 - y1 + 1, x2 - x1 + 1 , color_map, new_map);

	ili9341_send_color((void*)new_map, size * 2);	/*Send the remaining data*/
  free(new_map);
